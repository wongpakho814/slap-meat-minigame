...
const CheemsRender = () => {
  ...
  const startTimer = () => {
    setInterval(() => {
      ...
      updateTimePetted({ variables: { timePetted: seconds } });
    }, 1000);
  };
  ...
  return (
    <>
      {Auth.loggedIn() && (
        error ? (
            <span id="counter">Something went wrong when trying to connect to the Database!</span>
          ) : (
      ...
      <div>
        ...
        <img alt="cheems pic" src={playGif ? cheems10 : cheemsStatic} onClick={() => onClick()} />
      </div>
    </>
  );
};
